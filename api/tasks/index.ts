  ...
  try {
    ...
    if (request.method === 'POST') {
      ...
      if (!['admin', 'manager', 'branch_manager'].includes(userRole)) {
        response.status(403).json({ error: 'Only managers and admins can create tasks' });
        return;
      }

      const body = request.body as CreateTaskBody;
      const { title, description, assigned_agent_id, branch_id, priority, due_date } = body;
      ...
      const taskBranchId = branch_id || userBranchId;
